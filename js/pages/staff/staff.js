...
async function initSidebar() {
    const sidebarHtml = await fetchText("/pages/staff/sidebar.html");
    ...
    document
        ...
        .forEach((a) => {
            ...
            } else if (href.startsWith("/pages/admin/") && href.endsWith(".html")) {
                // bóc phần "/pages/admin/" để thành "orders/orders.html" hoặc "profile.html"
                const relative = href.replace(/^\/pages\/admin\//, "");
                a.addEventListener("click", (e) => {
                    e.preventDefault();
                    // cập nhật hash (vd "#orders/orders") và load trang
                    window.location.hash = relative.replace(/\.html$/, "");
                    loadPage(relative);
                });
            }
